...
class DynamixelDriver(DynamixelDriverProtocol):
    ...
    def _initialize_hardware(self):
        ...
        # Open once at the desired baudrate.
        # PortHandler.openPort() already calls setBaudRate/setupPort; calling
        # setBaudRate again would close+reopen and can hang on Windows FTDI.
        self._portHandler.baudrate = self._baudrate
        if not self._portHandler.openPort():
            raise RuntimeError(
                f"Failed to open the port {self._port} at {self._baudrate} baud"
            )
    ...
    def _check_port_availability(self) -> bool:
        ...
        # Windows: os.path.exists("COMx") is unreliable; lsof/fuser do not exist.
        if _IS_WINDOWS:
            try:
                from serial.tools import list_ports

                available = {p.device.upper() for p in list_ports.comports()}
                port_key = self._port.upper()
                if port_key not in available:
                    # Also accept bare "COM7" vs "\\\\.\\COM7" style names
                    bare = port_key.split("\\")[-1]
                    if bare not in available:
                        print(f"Port {self._port} not found in system COM list")
                        return False
                return True
            except Exception as e:
                print(f"Warning: could not enumerate COM ports ({e}); continuing")
                return True
    ...
    def _prepare_port(self):
        ...
        if not self._check_port_availability():
            if _IS_WINDOWS:
                print(
                    f"Warning: Port {self._port} was not listed; "
                    "close Dynamixel Wizard / other serial apps and retry"
                )
                return
